...
const base = (dm?: boolean) =>
  dm ? "mt-1.5 w-full rounded-lg border border-[#30363d] bg-[#0d1117] px-3 py-2 text-sm text-slate-100 focus:border-violet-500 focus:outline-none focus:ring-1 focus:ring-violet-500/50 transition-colors"
...
const NumberField = ({ field, value, onChange, error, darkMode }: FieldProps) => (
  <div className="w-full">
    <label className={lbl(darkMode)}>{field.label}{field.required && <span className="text-red-400 ml-0.5">*</span>}</label>
    <input type="number" value={typeof value === "number" || typeof value === "string" ? value : ""} onChange={(e) => onChange(e.target.value)} className={base(darkMode)} />

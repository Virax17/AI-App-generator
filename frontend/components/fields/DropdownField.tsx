...
const DropdownField = ({ field, value, onChange, error, darkMode }: FieldProps) => (
  <div className="w-full">
    <label className={lbl(darkMode)}>{field.label}{field.required && <span className="text-red-400 ml-0.5">*</span>}</label>
    <select value={typeof value === "string" ? value : ""} onChange={(e) => onChange(e.target.value)} className={base(darkMode)}>
